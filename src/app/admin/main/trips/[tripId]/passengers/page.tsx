...
export default async function AdminTripPassengersPage({
    ...
    if (!/^\d+$/.test(params.tripId)) notFound();
    const date =
        searchParams.date ||
    ...
    const response = await adminFetch(
        `/trip/trips/${params.tripId}/admin-details/?date=${encodeURIComponent(date)}`
    );
    ...
    if (!response.ok)
        return (
            <section className="mt-6 rounded-xl bg-white p-6" role="alert">
                ...
                <Link
                    className="underline"
                    href={`/admin/main/trips/${params.tripId}/passengers`}
                >
        ...
        <main className="mt-6 space-y-5 pb-10">
            <section className="rounded-xl bg-white p-6">
                <Link
                    ...
                    href={`/admin/main/trips/${params.tripId}`}
                >
                    К рейсу
                </Link>
